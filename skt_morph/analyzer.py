...
def _fetch_verbs_from_db(word_slp1):
    ...
    for row in cursor.fetchall():
        for vacana in _extract_exact_matches(row, word_slp1, ['eka', 'dvi', 'bahu']):
            results.append({"type": "verb", "dhatu_id": row["dhatu_id"], "upasarga": row["upasarga"], "derivative": row["derivative"], "prayoga": row["prayoga"], "lakara": row["lakara"], "voice": row["voice"], "purusha": row["purusha"], "vacana": vacana})
    conn.close()
    return results
...
def _fetch_participles_from_db(word_slp1):
    ...
    for row in cursor.fetchall():
        ...
        for col in _extract_exact_matches(row, word_slp1, ['base_form', 'masculine', 'feminine', 'neuter']):
            res = {"type": "avyaya" if pratyaya in avyaya_pratyayas else "participle", "dhatu_id": row["dhatu_id"], "upasarga": row["upasarga"], "derivative": row["derivative"], "pratyaya": pratyaya, "base_form": row["base_form"]}
            if col in ['masculine', 'feminine', 'neuter']: res.update({"gender": col, "case": "prathama", "vacana": "eka"})
            else: res["note"] = "Matched uninflected base form"
            results.append(res)
    conn.close()
    return results
...
def analyze_declension(word_slp1):
    ...
    conn = _get_db_connection()
    cursor = conn.cursor()
    for guess in get_stems(word_slp1):
        stem = guess["stem"]
        cursor.execute("SELECT dhatu_id, upasarga, pratyaya, base_form FROM participles WHERE base_form LIKE ?", (f"%{stem}%",))
        exact_matches = [row for row in cursor.fetchall() if stem in [s.strip() for s in row["base_form"].split(',')]] 
        if exact_matches:
            for row in exact_matches:
                results.append({"type": "declension", "base_form": stem, "gender": guess["gender"], "case": guess["case"], "vacana": guess["vacana"], "dhatu_id": row["dhatu_id"], "upasarga": row["upasarga"], "pratyaya": row["pratyaya"]})
        else:
            found_dynamic = False
            for upa, stripped_stem in get_upasarga_splits(stem):
                ...
                exact_dyn = [r for r in cursor.fetchall() if stripped_stem in [s.strip() for s in r["base_form"].split(',')]]
                if exact_dyn:
                    for r in exact_dyn:
                        results.append({"type": "declension", "base_form": stem, "gender": guess["gender"], "case": guess["case"], "vacana": guess["vacana"], "dhatu_id": r["dhatu_id"], "upasarga": upa, "pratyaya": r["pratyaya"], "note": "Dynamic Upasarga Match"})
                    found_dynamic = True
                    break
            if not found_dynamic:
                results.append({"type": "declension", "base_form": stem, "gender": guess["gender"], "case": guess["case"], "vacana": guess["vacana"], "dhatu_id": None, "upasarga": None, "pratyaya": None})
    conn.close()
    return results
